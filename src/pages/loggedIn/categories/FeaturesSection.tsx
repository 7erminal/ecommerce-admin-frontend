...
const FeaturesSection: React.FC = () => {
    ...
    return <div className="flex flex-col gap-4">
        <Panel
            title="Features"
            description="Highlight what makes each product special."
            action={
        ...
        >
            {features.length === 0 ? (
                <EmptyState
                    ...
                    hint={"Use \"Add Feature\" to create your first one."}
                />
            ) : (
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    {features.map((entry) => (
                        <div
                            key={entry.FeatureId.toString()}
                        ...
                        >
                            <span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-red-50 text-sm font-semibold text-[#c53030]">
                                {entry.ImagePath ? (
                                    <img src={entry.ImagePath} alt={entry.FeatureName} className="h-full w-full object-cover" />
                                ) : (
                                    <span>{(entry.FeatureName || "?").charAt(0).toUpperCase()}</span>
                                )}
                            </span>
                            <div className="min-w-0 flex-1">
                                <div className="truncate text-sm font-semibold text-gray-800">{entry.FeatureName}</div>
