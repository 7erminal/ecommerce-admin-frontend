...
const CategoriesSection: React.FC = () => {
    ...
    return <div className="flex flex-col gap-4">
        <Panel
            title="Categories"
            description="Group your products so shoppers can browse with ease."
            action={
        ...
        >
            {categories.length === 0 ? (
                <EmptyState
                    ...
                    hint={"Use \"Add Category\" to create your first one."}
                />
            ) : (
                    ...
                    {categories.map((entry) => (
                        ...
                        >
                            <span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-red-50 text-sm font-semibold text-[#c53030]">
                                {entry.ImagePath ? (
                                    <img src={entry.ImagePath} alt={entry.CategoryName} className="h-full w-full object-cover" />
                                ) : (
                                    <span>{(entry.CategoryName || "?").charAt(0).toUpperCase()}</span>
                                )}
                            </span>
                            <div className="min-w-0 flex-1">
                                <div className="truncate text-sm font-semibold text-gray-800">{entry.CategoryName}</div>
