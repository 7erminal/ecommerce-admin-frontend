...
import type { AddFeature, Feature } from "../../../../resources/types/applicationTypes";
...
const FeaturesSection: React.FC = () => {
    ...
    const openEdit = (entry: Feature) => {
        setEditingId(entry.FeatureId.toString());
        setName(entry.FeatureName);
    ...
    const handleSave = async () => {
        ...
        const resp = await appContext?.addFeature(payload);
        if (resp?.Success) {
    ...
    const handleDelete = async () => {
        ...
        const resp = await appContext?.deleteFeature(editingId.toString());
        if (resp?.Success) {
            // Handle success (e.g., show a success message, refresh the list)
        } else {
            // Handle error (e.g., show an error message)
    ...
    return <div className="p-6">
        <section className="mb-6">
            ...
            <div className="space-y-4">
                {appContext?.features.map((entry, index) => (
                    <div key={entry.FeatureId.toString()} className={`flex items-start gap-4 p-4 ${index !== appContext.features.length - 1 ? "border-b border-gray-200" : ""}`}>
                        <img src={entry.ImagePath ? entry.ImagePath : ""} alt={entry.FeatureName} className="w-16 h-16 rounded-lg object-cover" />
                        <div className="flex-1">
                            <div className="text-sm font-semibold text-gray-800">{entry.FeatureName}</div>
